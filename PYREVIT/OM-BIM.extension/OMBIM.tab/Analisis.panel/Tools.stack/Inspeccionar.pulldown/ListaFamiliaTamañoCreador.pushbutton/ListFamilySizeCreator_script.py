...
__title__ = 'Lista Familia Tamaño'
__authors__ = ['Oscar Mendoza']

FIELDS = ["Tamaño", "Nombre", "Categoria", "Creador"]
...
def print_totals(families):
    total_size = sum([fam_item.get("Tamaño") or 0 for fam_item in families])
    print("%d Familias, Tamaño total encontrad: %s\n\n" % (
...
def print_sorted(families, group_by):
    ...
    if group_by not in ["Creador", "Categoria"]: # do not group by name and size
        print("Sort by: %s" % group_by)
        print("; ".join(fields_sorted))
        families_grouped = {"": sorted(families, key=lambda fam_item: fam_item.get(group_by),
                                       reverse=group_by=="Tamaño")}
    else:
        ...
        families_grouped = {}
        # reverse if sorted by Tamaño
        for fam_item in sorted(families, key=lambda fam_item: fam_item.get(sort_by),
                               reverse=sort_by=="Tamaño"):
    ...
    for group_value in sorted(families_grouped.keys()):
        ...
        for fam_item in families_grouped[group_value]:
            ...
            for field in fields_sorted:
                ...
                family_row.append(str(value))  # Ensure value is a string
            print("; ".join(family_row))
        print_totals(families_grouped[group_value])

# main logic
# ask user to choose sort option
sort_by = forms.CommandSwitchWindow.show(FIELDS,
     message='Tipo de filtrados:',
)
if not sort_by:
    script.exit()
    ...
    for fam in all_fams:
        with revit.ErrorSwallower() as swallower:
            if fam.IsEditable:
                ...
                all_family_items.append({"Tamaño": fam_size,
                                         "Creador": fam_creator,
                                         "Categoria": fam_category,
                                         "Nombre": fam.Name})
